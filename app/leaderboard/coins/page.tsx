        ...
        {lastWeekList.length > 0 && (
          <div className="mb-6 p-4 bg-gradient-to-b from-amber-500/20 via-amber-500/5 to-transparent border border-amber-500/30 rounded-2xl text-center shadow-inner">
            ...
            <div className="grid grid-cols-3 gap-2 items-end">
              {top2 ? <PodiumCard rank={2} user={top2} suffix="Coins" /> : <div />}
              {top1 ? <PodiumCard rank={1} user={top1} suffix="Coins" /> : <div />}
              {top3 ? <PodiumCard rank={3} user={top3} suffix="Coins" /> : <div />}
            </div>
          </div>
        )}
        ...
        {currentList.length > 0 ? (
          <div className="space-y-2.5">
            {currentList.map((user, i) => (
              <ListCard key={user.user_id} rank={i + 1} user={user} suffix="coins" />
            ))}
          </div>
        ) : (
