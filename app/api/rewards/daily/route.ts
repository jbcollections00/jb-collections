...
async function loadRecentDailyRewardDates(adminDb: ReturnType<typeof createSupabaseAdminClient>, userId: string) {
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      streakBonus: claimRow ? Math.max(Number(claimRow.amount || 0) - baseCoins, 0) : 0,
      rewardDate: today,
...
export async function POST() {
  try {
    ...
    const newStreak = currentStreakInfo.claimedYesterday ? currentStreakInfo.streak + 1 : 1
