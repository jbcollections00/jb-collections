...
export default function AdminHeader() {
  ...
  return (
    <header className="w-full px-3 pt-3 sm:px-4 sm:pt-4">
      <div className="mx-auto w-full max-w-[1800px] rounded-[24px] border border-slate-800/80 bg-slate-950/95 shadow-[0_16px_40px_rgba(2,6,23,0.55)] backdrop-blur-xl">
        <div className="flex min-h-[72px] items-center justify-between gap-3 px-4 py-2.5 sm:px-5 lg:px-6">
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link
                  key={item.href}
                  href={item.href}
                  className={`inline-flex items-center gap-2 rounded-full border px-3.5 py-2 text-xs font-bold whitespace-nowrap transition ${
                    active
          ...
          <div className="hidden shrink-0 items-center lg:flex">
            <button
              ...
              disabled={loggingOut}
              className="inline-flex h-10 items-center justify-center rounded-full border border-red-400/20 bg-red-600 px-5 text-xs font-bold text-white transition hover:bg-red-700 disabled:cursor-not-allowed disabled:opacity-70"
            >
